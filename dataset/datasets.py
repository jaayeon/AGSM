...
class EncodedDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.model != 'sd1.5':
            ...
            if "hardneg" in self.encoding_dir:
                neg_prompt_emb = torch.load(osp.join(self.encoding_dir, f"neg_prompt_emb_{index}.pt"), map_location="cpu")
                if len(neg_prompt_emb.shape) == 4:
                    neg_prompt_emb = neg_prompt_emb[0]
                neg_prompt_emb = neg_prompt_emb.detach()

                neg_pooled_prompt_emb = torch.load(osp.join(self.encoding_dir, f"neg_pooled_prompt_emb_{index}.pt"), map_location="cpu")
                if len(neg_pooled_prompt_emb.shape) == 3:
                    neg_pooled_prompt_emb = neg_pooled_prompt_emb[0]
                neg_pooled_prompt_emb = neg_pooled_prompt_emb.detach()
                return latent, (prompt_emb, pooled_prompt_emb), (neg_prompt_emb, neg_pooled_prompt_emb)
            else:
