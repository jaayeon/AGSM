...
from transformer import SD3Transformer2DModel_AGSM, SD15UNet2DConditionModel_AGSM, SDXLUNet2DConditionModel_AGSM
...
class SD3EulerAGSM(SD3Euler):
    def __init__(self, model_key:str='stabilityai/stable-diffusion-3-medium-diffusers', device='cuda', n_soft_tokens:int=4, use_soft_t=True, n_soft_layers:int=6):
        super().__init__(model_key=model_key, device=device)

        custom_transformer = SD3Transformer2DModel_AGSM(self.denoiser, n_soft_tokens=n_soft_tokens, use_soft_t=use_soft_t, n_soft_layers=n_soft_layers)
        ema = SD3Transformer2DModel_AGSM(self.denoiser, n_soft_tokens=n_soft_tokens, use_soft_t=use_soft_t, n_soft_layers=n_soft_layers)
        
        self.denoiser = custom_transformer.to(device)
        self.denoiser.requires_grad_(False)
        self.denoiser.eval()
        self.ema = ema.to(device)
        self.ema.requires_grad_(False)
        self.ema.eval()

    def initialize_soft_tokens(self, soft_tokens, soft_t_tokens=None):
        self.ema.initialize_soft_tokens(soft_tokens=soft_tokens, soft_t_tokens=soft_t_tokens)

    @torch.no_grad()
    def sample(self, prompts: List[str], NFE:int, prompt_emb=None, null_prompt_emb=None, img_shape: Optional[Tuple[int]]=None, 
               cfg_scale: float=1.0, batch_size: int = 1, latent:Optional[torch.Tensor]=None, use_soft_tokens:bool=False, use_neg=False, use_ema=False, uncon2con=False):
        imgH, imgW = img_shape if img_shape is not None else (1024, 1024)
        ...
        for i, t in enumerate(timesteps):
            ...
            pred_v = self.predict_vector(z, timestep, prompt_emb, pooled_emb, use_soft_tokens=use_soft_tokens, use_ema=use_ema)
            if cfg_scale != 1.0:
                if use_neg:
                    neg = torch.ones(z.shape[0]).long() # negative policy
                else:
                    neg = torch.zeros(z.shape[0]).long() # positive policy
                if uncon2con:
                    pred_null_v = self.predict_vector(z, timestep, prompt_emb, pooled_emb, use_soft_tokens=use_soft_tokens, neg=neg, use_ema=use_ema) # con
                else:
                    pred_null_v = self.predict_vector(z, timestep, null_prompt_emb, null_pooled_emb, use_soft_tokens=use_soft_tokens, neg=neg, use_ema=use_ema) # uncon
    ...
    def predict_vector(self, z, t, prompt_emb, pooled_emb, use_soft_tokens=False, use_ema=False, neg=None):
        if use_ema:
            v = self.ema(hidden_states=z,
                             timestep=t,
                             pooled_projections=pooled_emb,
                             encoder_hidden_states=prompt_emb,
                             return_dict=False,
    ...
    def error(self, latent, nidxs, pidxs, prompts_embs, t, noise=None, use_soft_tokens=False, get_ema=False, neg=False):
        ...
        pred_v = self.predict_vector(zt, timestep, prompt_emb, pooled_emb, use_soft_tokens=use_soft_tokens, neg=neg)
        v = noise-latent
        if get_ema:
            ema_v = self.predict_vector(zt, timestep, prompt_emb, pooled_emb, use_soft_tokens=use_soft_tokens, use_ema=True, neg=neg)
            return v, pred_v, ema_v
        else:
            return v, pred_v
...
class StableDiffusion1Base():
    def __init__(self, model_key:str='stable-diffusion-v1-5/stable-diffusion-v1-5', device='cuda', dtype=torch.float16):
        self.device = device
...
class SD1EulerAGSM(SD1Euler):
    def __init__(self, model_key:str='stable-diffusion-v1-5/stable-diffusion-v1-5', device='cuda', n_soft_tokens:int=4, use_soft_t=True, apply_soft_tokens=[True, True, False]):
        super().__init__(model_key=model_key, device=device)

        custom_unet = SD15UNet2DConditionModel_AGSM(self.denoiser, n_soft_tokens=n_soft_tokens, use_soft_t=use_soft_t, apply_soft_tokens=apply_soft_tokens)
        ema = SD15UNet2DConditionModel_AGSM(self.denoiser, n_soft_tokens=n_soft_tokens, use_soft_t=use_soft_t, apply_soft_tokens=apply_soft_tokens)
    ...
    def initialize_soft_tokens(self, soft_tokens, soft_t_tokens=None):
        self.denoiser.initialize_soft_tokens(soft_tokens=soft_tokens, soft_t_tokens=soft_t_tokens)

    @torch.no_grad()
    def sample(self, prompts: List[str], NFE:int, prompt_emb=None, null_prompt_emb=None, img_shape: Optional[Tuple[int]]=None, 
               cfg_scale: float=1.0, batch_size: int = 1, latent:Optional[torch.Tensor]=None, use_soft_tokens:bool=False, use_ema=False, use_neg=False, uncon2con=False):
        imgH, imgW = img_shape if img_shape is not None else (1024, 1024)
        ...
        for i, t in enumerate(timesteps):
            timestep = t.expand(z.shape[0]).to(self.denoiser.device)
            pred_epsilon = self.predict_vector(z, timestep, prompt_emb, use_soft_tokens=use_soft_tokens, use_ema=use_ema)
            if cfg_scale != 1.0:
                if use_neg:
                    neg = torch.ones(z.shape[0]).long() # negative policy
                else:
                    neg = torch.zeros(z.shape[0]).long() # positive policy
                if uncon2con:
                    pred_null_epsilon = self.predict_vector(z, timestep, prompt_emb, use_soft_tokens=use_soft_tokens, use_ema=use_ema, neg=neg)
                else:
                    pred_null_epsilon = self.predict_vector(z, timestep, null_prompt_emb, use_soft_tokens=use_soft_tokens, use_ema=use_ema, neg=neg)
            else:
                pred_null_epsilon = 0.0
    ...
    def predict_vector(self, z, t, prompt_emb, use_soft_tokens=False, use_ema=False, neg=None):
        if use_ema:
            v = self.ema(z,
                    timestep=t,
                    encoder_hidden_states=prompt_emb,
    ...
    def error(self, latent, nidxs, pidxs, prompts_embs, t, noise=None, use_soft_tokens=False, get_ema=False, neg=False):
        ...
        timestep = t.expand(noise.shape[0]).to(self.denoiser.device)
        zt = self.scheduler.add_noise(latent, noise, t)
        pred_epsilon = self.predict_vector(zt, timestep, prompt_emb, use_soft_tokens=use_soft_tokens, neg=neg)
        epsilon = noise

        if get_ema:
            ema_pred_epsilon = self.predict_vector(zt, timestep, prompt_emb, use_soft_tokens=use_soft_tokens, use_ema=True, neg=neg)
            return epsilon, pred_epsilon, ema_pred_epsilon
        else:
            return epsilon, pred_epsilon
    

class StableDiffusionXLBase():
    def __init__(self, model_key:str='stabilityai/stable-diffusion-xl-base-1.0', device='cuda', dtype=torch.float16):
        self.device = device
...
class SDXLEuler(StableDiffusionXLBase):
    def __init__(self, model_key:str='stabilityai/stable-diffusion-xl-base-1.0', device='cuda'):
        super().__init__(model_key=model_key, device=device)
...
class SDXLEulerAGSM(SDXLEuler):
    def __init__(self, model_key:str='stabilityai/stable-diffusion-xl-base-1.0', device='cuda', n_soft_tokens:int=4, use_soft_t=True, apply_soft_tokens=[True, True, False]):
        super().__init__(model_key=model_key, device=device)

        custom_unet = SDXLUNet2DConditionModel_AGSM(self.denoiser, n_soft_tokens=n_soft_tokens, use_soft_t=use_soft_t, apply_soft_tokens=apply_soft_tokens)
        ema = SDXLUNet2DConditionModel_AGSM(self.denoiser, n_soft_tokens=n_soft_tokens, use_soft_t=use_soft_t, apply_soft_tokens=apply_soft_tokens)
    ...
    def sample(self, prompts: List[str], NFE:int, prompt_emb=None, null_prompt_emb=None, img_shape: Optional[Tuple[int]]=None, 
        ...
        for i, t in enumerate(timesteps):
            timestep = t.expand(z.shape[0]).to(self.denoiser.device)
            pred_epsilon = self.predict_vector(z, timestep, prompt_emb, pooled_prompt_emb, add_time_ids, use_soft_tokens=use_soft_tokens)
            if cfg_scale != 1.0:
                if use_neg:
                    neg = torch.ones(z.shape[0]).long() # negative policy
                else:
                    neg = torch.zeros(z.shape[0]).long() # positive policy
                if uncon2con:
                    pred_null_epsilon = self.predict_vector(z, timestep, prompt_emb, pooled_prompt_emb, add_time_ids, use_soft_tokens=use_soft_tokens, neg=neg)
                else:
                    pred_null_epsilon = self.predict_vector(z, timestep, null_prompt_emb, null_pooled_prompt_emb, add_time_ids, use_soft_tokens=use_soft_tokens, neg=neg)
            else:
                pred_null_epsilon = 0.0
    ...
    def predict_vector(self, z, t, prompt_emb, pooled_prompt_emb, add_time_ids, use_soft_tokens=False, use_ema=False, neg=None):
    ...
    def error(self, latent, nidxs, pidxs, prompts_embs, t, noise=None, use_soft_tokens=False, get_ema=False, neg=False):
        ...
        zt = self.scheduler.add_noise(latent, noise, t)
        pred_epsilon = self.predict_vector(zt, timestep, prompt_emb, pooled_prompt_emb, add_time_ids, use_soft_tokens=use_soft_tokens, neg=neg)
        epsilon = noise

        if get_ema:
            ema_pred_epsilon = self.predict_vector(zt, timestep, prompt_emb, pooled_prompt_emb, add_time_ids, use_soft_tokens=use_soft_tokens, use_ema=True, neg=neg)
            return epsilon, pred_epsilon, ema_pred_epsilon
        else:
            return epsilon, pred_epsilon
